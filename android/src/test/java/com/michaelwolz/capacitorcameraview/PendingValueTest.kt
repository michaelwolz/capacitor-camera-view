 ...
 * made before the camera is attached.
 *
...
class PendingValueTest {
    ...
    @Test
    fun `propagateIfPresent is a no-op when nothing is buffered`() {
    ...
    @Test
    fun `a buffered value is applied and then forgotten`() {
        val pending = PendingValue<Float>()
    ...
    @Test
    fun `the callback is only invoked once the buffered value is applied`() {
        val pending = PendingValue<Float>()
        ...
        pending.propagateIfPresent { _, callback -> callback(null) }
    ...
    @Test
    fun `clear is a no-op when nothing is buffered`() {
        val pending = PendingValue<Float>()

        pending.clear(CameraError.CameraNotInitialized())
